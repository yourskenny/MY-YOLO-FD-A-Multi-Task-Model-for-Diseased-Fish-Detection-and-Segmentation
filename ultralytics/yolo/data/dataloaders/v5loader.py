...
from .v5augmentations import (Albumentations, SegAlbumentations, augment_hsv, classify_albumentations,
                              classify_transforms, copy_paste,
                              letterbox, segletterbox, mixup, random_perspective, random_segmentation_perspective,
                              letterboxandsegLabel)
...
# InfiniteDataLoader在遍历完数据集后不会引发StopIteration异常，而是无限循环回到数据集的开头。
...
class LoadImagesAndLabels(Dataset):
    ...
    def __init__(self,
            ...
            for p in path if isinstance(path, list) else [path]:
                ...
                if p.is_dir():  # dir
                    ...
                    with open(p) as t:
                        ...
                        f.append([x.replace('./', parent, 1) if x.startswith('./') else x for x in t])
                        # f += [p.parent / x.lstrip(os.sep) for x in t]  # to global path (pathlib)
                else:
                    raise FileNotFoundError(f'{prefix}{p} does not exist')
            def filter_images(file_list):
                return sorted(x.replace('/', os.sep) for x in file_list if x.split('.')[-1].lower() in IMG_FORMATS)

            self.im_files = filter_images(f[0])
            f[1] = filter_images(f[1])
        ...
        self.val = not augment
        self.seg_weights, segcls = self.get_seg_weights(os.path.dirname(self.seg_label_files[0]))
        # update label map
        ...
        if min_items:
            ...
            self.seg_label_files = [self.seg_label_files[i] for i in include]
        ...
        if self.rect:
            ...
            self.seg_label_files = [self.seg_label_files[i] for i in irect]
    ...
    def get_seg_weights(self, path):
        """ path : seg_labels path"""
        class_weights_cache = os.path.join(os.path.dirname(path), "class_weights.cache")
        if not os.path.exists(class_weights_cache):
            labelcounter = Counter()
            for img in self.seg_label_files:
                image = cv2.imread(img, cv2.IMREAD_GRAYSCALE)
                num = image.flatten().tolist()
                labelcounter.update(num)

            labelcounter = dict(labelcounter)
            segcls = sorted(list(labelcounter.keys()))[:2]
            seg_weights = [labelcounter[i] for i in segcls]
            seg_weights_balance = [math.pow(i, 1 / 5) for i in seg_weights]
            seg_weights = [max(seg_weights_balance) / i for i in seg_weights_balance]
            with open(class_weights_cache, 'wb') as file:
                pickle.dump(segcls, file)
                pickle.dump(seg_weights, file)
        else:
            with open(class_weights_cache, 'rb') as file:
                segcls = pickle.load(file)
                seg_weights = pickle.load(file)
        return seg_weights, segcls
    ...
    @staticmethod
    def convert_label(label_mapping, label, inverse=False):
        temp = label.copy()
        if inverse:
    ...
    def __getitem__(self, index):
        ...
        if mosaic:
            # Load mosaic
            img, labels, seg_label = self.load_mosaic(index)
            shapes = None
            ...
            # Load image
            img, seg_label, (h0, w0), (h, w) = self.load_image(index)

            # Letterbox
            shape = self.batch_shapes[self.batch[index]] if self.rect else self.img_size  # final letterboxed shape
            img, seg_label, ratio, pad = letterboxandsegLabel(img, seg_label, shape, auto=False, scaleup=self.augment)
            ...
            if self.augment:
                img, labels, seg_label = random_perspective(img,
                                                 labels,
                                                 seg_label,
                                                 degrees=hyp['degrees'],
        ...
        if self.augment:
            # Albumentations
            img, labels, seg_label = self.albumentations(img, labels, seg_label)
        ...
        seg_label = self.convert_label(self.label_mapping, seg_label)
        img = np.ascontiguousarray(img)
        seg_label = np.ascontiguousarray(seg_label)

        return torch.from_numpy(img), [labels_out, torch.from_numpy(seg_label)], self.im_files[index], shapes

    def load_image(self, i):
        """Loads 1 image from dataset index 'i', returns (im, original hw, resized hw)."""
        im, f, fn = self.ims[i], self.im_files[i], self.npy_files[i],
        seg_lpath = self.seg_label_files[i]
        seg_label = cv2.imread(seg_lpath, cv2.IMREAD_GRAYSCALE)

        assert seg_label is not None, f'Image Not Found {seg_label}'
        ...
        if im is None:  # not cached in RAM
            ...
            if r != 1:  # if sizes are not equal
                ...
                seg_label = cv2.resize(seg_label, (math.ceil(w0 * r), math.ceil(h0 * r)), interpolation=cv2.INTER_NEAREST)
    ...
    def load_mosaic(self, index):
        ...
        for i, index in enumerate(indices):
            # Load image
            img, seg_label, _, (h, w) = self.load_image(index)

            # Place img in img4
            if i == 0:  # top left
                img4 = np.full((s * 2, s * 2, img.shape[2]), 114, dtype=np.uint8)  # base image with 4 tiles
                seg_label4 = np.full((s * 2, s * 2), 0, dtype=np.uint8)
            ...
            # seg labels
            seg_label4[y1a:y2a, x1a:x2a] = seg_label[y1b:y2b, x1b:x2b]
        ...
        img4, labels4, seg_label4 = random_perspective(img4,
                                           labels4,
                                           seg_label4,
                                           segments4,
        ...
        return img4, labels4, seg_label4
    ...
    def collate_fn(batch):
        ...
        im, labels, path, shapes = zip(*batch)  # transposed
        det_label, seg_label = zip(*labels)
        seg_label = torch.stack(seg_label)
        for i, lb in enumerate(det_label):
            lb[:, 0] = i  # add target image index for build_targets()
        batch_idx, cls, bboxes = torch.cat(det_label, 0).split((1, 1, 4), dim=1)
        return {
...
class LoadSegImagesAndLabels(Dataset):
    ...
    def __init__(self, path, cls_num=20, img_size=640, batch_size=16, augment=False, hyp=None, rect=False,
        ...
        class_weights_cache = os.path.join(os.path.dirname(os.path.dirname(self.label_files[0])), "class_weights.cache")
        if not os.path.exists(class_weights_cache) and not self.val:
            # TODO 太消耗算力了, 优化读取cache
            labelcounter = Counter()
            for img in self.label_files:
                image = cv2.imread(img, cv2.IMREAD_GRAYSCALE)
                num = image.flatten().tolist()
            ...
            with open(class_weights_cache, 'wb') as file:
                pickle.dump(segcls, file)
                pickle.dump(self.seg_weights, file)
        else:
            with open(class_weights_cache, 'rb') as file:
                segcls = pickle.load(file)
                self.seg_weights = pickle.load(file)
        # update label map
        for i, cls in enumerate(segcls):
            self.label_mapping[cls] = i
