...
def validate(testloader, model, numcls, criterion):
    ...
    with torch.no_grad():
        for idx, batch in enumerate(pbar):
            ...
            size = label.size()
            image = image.to(device).float() / 255
...
def validation(testloader, model, numcls, device, fp16=True, ave_loss=None):
    ...
    try:
        ...
        LOGGER.error(f"Error in segmentation validation: {e}")
        import traceback
    ...
    if ave_loss is not None:
        if hasattr(ave_loss, 'average'):
            val_loss = ave_loss.average()
        elif hasattr(ave_loss, 'item'):
            val_loss = ave_loss.item()
        else:
            val_loss = float(ave_loss)
        LOGGER.info('%11g' * 3 % (len(testloader), val_loss, mean_IoU))
    else:
