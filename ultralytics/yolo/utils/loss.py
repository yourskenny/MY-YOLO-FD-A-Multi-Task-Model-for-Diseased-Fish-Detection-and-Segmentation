...
class MulticlassVarifocalLoss(nn.Module):
    def __init__(self, alpha=None, gamma=3, reduction='mean'):
        super(MulticlassVarifocalLoss, self).__init__()
    ...
    def forward(self, inputs, targets):
        # 将模型输出调整形状以匹配真实标签
        inputs = inputs.permute(0, 2, 3, 1).contiguous().view(-1, inputs.size(1))  # (8*640*640, 21)
        targets = targets.view(-1)  # (8*640*640,)
        alpha = self.alpha[targets]
        # 计算交叉熵损失
        ce_loss = F.cross_entropy(inputs, targets, reduction='none')
        # 计算 Focal 系数
        pt = torch.exp(-ce_loss)
        # at = self.alpha * (1 - pt) ** self.gamma
        at = alpha * (1.5 - pt) ** self.gamma
        # 计算 Varifocal Loss
        loss = at * ce_loss
...
class DiceLoss(nn.Module):
    ...
    def forward(self, inputs, targets):
        ...
        for cls in range(1, self.num_classes):  # 排除背景类 0
            target = (targets == cls).float()
            pred = torch.sigmoid(inputs[:, cls, :, :])
...
class CombinedLoss(nn.Module):
    def __init__(self, alpha=None, gamma=3, reduction='mean',num_classes=2):
        super(CombinedLoss, self).__init__()
        self.dice_loss = DiceLoss(num_classes)
        self.varifocal_loss = MulticlassVarifocalLoss(alpha, gamma, reduction)

    def forward(self, inputs, targets):
        dice = self.dice_loss(inputs, targets)
        varifocal = self.varifocal_loss(inputs, targets)
        combined_loss = dice + varifocal  # 或者可以根据实际情况加权求和
...
class v8DetectionLoss:
    ...
    def __init__(self, model):  # model must be de-paralleled
        ...
        m = [m_ for m_ in model.model if isinstance(m_, Detect)][0]
        # m = model.model[len(model.model)-9-1]  # Detect() module
