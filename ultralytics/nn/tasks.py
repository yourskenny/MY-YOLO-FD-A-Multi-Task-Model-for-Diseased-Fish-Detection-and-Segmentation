...
from ultralytics.nn.modules import (AIFI, C1, C2, C3, C3TR, SPP, SPPF, Bottleneck, BottleneckCSP, C2f, C3Ghost, C3x,
                                    ...
                                    RTDETRDecoder, Segment, SwinTransformer, SwinTransformerB, SwinTransformerC,
                                    ContextAggregation, BiLevelRoutingAttention, Attention, AttentionLePE,
                                    SimFusion_4in, SimFusion_3in, SimFusion_3in_bottom, IFM, InjectionMultiSum_Auto_pool,
                                    CBAM)
from ultralytics.nn.modules.GD import PyramidPoolAgg, TopBasicLayer, AdvPoolFusion
...
class BaseModel(nn.Module):
    ...
    def _predict_once(self, x, profile=False, visualize=False):
        ...
        outputs = []
        # out_idx = [22,31]
        # 当使用AMP检测时,会导入yolov8n.pt进行测试,这里的out_idx应该考虑这种情况，或者直接设置amp=False
        if len(self.model) == 23:
            out_idx = [22, 31]
        else:
            # out_idx = [24, 33]
            out_idx = [len(self.model)-9-1,len(self.model)-1]
        # print("-------------------------------------")
    ...
    def loss(self, batch, preds=None):
        ...
        return self.criterion(self.predict(batch['img'])[0] if preds is None else preds, batch)
...
class DetectionModel(BaseModel):
    ...
    def __init__(self, cfg='yolov8n.yaml', ch=3, nc=None, verbose=True):  # model, input channels, number of classes
        ...
        # Define Seg Loss, need init in trainer.py
        self.SegLoss = None

        # Build strides
        # m = self.model[-1]  # Detect()
        det_ind= len(self.model) - len(self.yaml['segHead']) - 1
        m=self.model[det_ind] # Detect()
        if isinstance(m, (Detect, Segment, Pose)):
            ...
            m.bias_init()  # TODO only run once ???是否注释
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head'] + d['segHead']):  # from, number, module, args
        ...
        if m in (Classify, Conv, ConvTranspose, GhostConv, Bottleneck, GhostBottleneck, SPP, SPPF, DWConv, Focus,
            ...
            if m is HGBlock:
                ...
                n = 1
        elif m in (BiLevelRoutingAttention,Attention,AttentionLePE,CBAM,ContextAggregation):    # 添加Biformer注意力机制、CBAM和Ctxt模块
            c2 = ch[f]
            args = [c2, *args]
            ...
            c2 = sum(ch[x] for x in f)
        elif m in (SimFusion_3in, SimFusion_3in_bottom):
            c2 = args[0]
            if c2 != nc:  # if c2 not equal to number of classes (i.e. for Classify() output)
                c2 = make_divisible(min(c2, max_channels) * width, 8)
...
def yaml_model_load(path):
    ...
    yaml_file = check_yaml(unified_path, hard=False) or check_yaml(path) # 'C:\\Users\\zhaor\\PycharmProjects\\ultralytics-main\\ultralytics\\models\\v8\\yolov8.yaml'
